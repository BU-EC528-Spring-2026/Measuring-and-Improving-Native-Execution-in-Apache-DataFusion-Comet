#!/bin/bash
# run_demo.sh  — runs TPC-H Q1-Q22 with Comet enabled
...
// Load tables
Seq("customer","lineitem","orders","part","partsupp","supplier","nation","region").foreach { t =>
  spark.read.parquet(s"$parquetDir/${t}_parquet").createOrReplaceTempView(t)
}
...
val tpch = Seq(
  ...
  "Q6" ->"""select sum(l_extendedprice * l_discount) as revenue from lineitem where l_shipdate >= date '1994-01-01' and l_shipdate < date '1994-01-01' + interval '1' year and l_discount between .06 - 0.01 and .06 + 0.01 and l_quantity < 24""",
