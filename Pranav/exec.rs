...
impl SortMergeJoinExec {
    ...
    /// Public constructor that takes a boolean flag for filtered SMJ mode
    pub fn try_new_with_mode(
        ...
        let mode = if pranav_enabled {
            FilteredSmjMode::Pranav
        } else {
            FilteredSmjMode::Legacy
        };
        Self::try_new_with_mode_internal(
            left,
            right,
            on,
            filter,
    ...
    /// Pranav's code
    pub(crate) fn try_new_with_mode_internal(
        left: Arc<dyn ExecutionPlan>,
        right: Arc<dyn ExecutionPlan>,
        on: JoinOn,
        filter: Option<JoinFilter>,
    ...
    pub fn swap_inputs(&self) -> Result<Arc<dyn ExecutionPlan>> {
        ...
        let new_join = SortMergeJoinExec::try_new_with_mode_internal(
            Arc::clone(right),
            Arc::clone(left),
            self.on()
                .iter()
...
impl ExecutionPlan for SortMergeJoinExec {
    ...
    fn with_new_children(
        ...
        match &children[..] {
            [left, right] => Ok(Arc::new(SortMergeJoinExec::try_new_with_mode_internal(
                Arc::clone(left),
                Arc::clone(right),
                self.on.clone(),
                self.filter.clone(),
    ...
    fn try_swapping_with_projection(
        ...
        Ok(Some(Arc::new(SortMergeJoinExec::try_new_with_mode_internal(
            Arc::new(new_left),
            Arc::new(new_right),
            new_on,
            self.filter.clone(),
