...
use datafusion::{
    ...
    physical_plan::{
        ...
        empty::EmptyExec,
        joins::{
            utils::JoinFilter, CrossJoinExec, HashJoinExec, NestedLoopJoinExec, PartitionMode,
            SortMergeJoinExec
        },
...
use datafusion_comet_spark_expr::{
    ...
    RandnExpr, SparkCastOptions, MaxMinBy, MaxMinByKind, Stddev, SumDecimal, ToJson, UnboundColumn, Variance,
};
...
impl PhysicalPlanner {
    ...
    pub(crate) fn create_plan<'a>(
        ...
        match spark_plan.op_struct.as_ref().unwrap() {
            ...
            OpStruct::HashAgg(agg) => {
                ...
                let aggr_expr: Vec<Arc<AggregateFunctionExpr>> =
                ...
                let aggregate: Arc<dyn ExecutionPlan> = Arc::new(aggregate_exec);
            ...
            OpStruct::SortMergeJoin(join) => {
                ...
                let join = Arc::new(SortMergeJoinExec::try_new_with_mode(
                    ...
                    join.pranav_filtered_smj_enabled,
                )?);
            ...
            OpStruct::CartesianProduct(join) => {
                assert_eq!(children.len(), 2);

                let (mut left_scans, left) =
                ...
                let left_plan = Arc::clone(&left.native_plan);
                let right_plan = Arc::clone(&right.native_plan);

                let join_filter =
                    self.parse_cartesian_condition(&left, &right, &join.condition)?;

                if let Some(filter) = join_filter {
                    let nested_loop = Arc::new(NestedLoopJoinExec::try_new(
                        left_plan,
                        right_plan,
                        Some(filter),
                        &DFJoinType::Inner,
                    ...
                    let left_single: Arc<dyn ExecutionPlan> =
                        Arc::new(CoalescePartitionsExec::new(left_plan));
                    let cross_join = Arc::new(CrossJoinExec::new(left_single, right_plan));
    ...
    fn parse_cartesian_condition(
        &self,
        left: &Arc<SparkPlan>,
        right: &Arc<SparkPlan>,
        condition: &Option<Expr>,
    ) -> Result<Option<JoinFilter>, ExecutionError> {
        let join_filter = if let Some(expr) = condition {
            let left_schema = left.schema();
            let right_schema = right.schema();
            let left_fields = left_schema.fields();
            ...
            let fields: Vec<_> = full_schema
                ...
                .map(|f| match f.data_type() {
                    DataType::Dictionary(_, value_type) => {
                        Arc::new(Field::new(f.name(), value_type.as_ref().clone(), f.is_nullable()))
                    }
                    _ => Arc::clone(f),
                })
                .collect();
            ...
            let filter_expr = self.create_expr(expr, Arc::clone(&intermediate_schema))?;

            let left_indices: Vec<usize> = (0..left_schema.fields().len()).collect();
            let right_indices: Vec<usize> = (0..right_schema.fields().len()).collect();
            let column_indices = JoinFilter::build_column_indices(left_indices, right_indices);
