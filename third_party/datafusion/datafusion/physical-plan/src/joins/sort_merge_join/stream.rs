...
use datafusion_physical_expr::expressions::{BinaryExpr, CastExpr, Column, TryCastExpr};
...
fn classify_semi_anti_fast_path(
    ...
    let mut cross_spec: Option<CrossComparatorSpec> = None;
    let mut right_prefilter: Option<RightOnlyComparatorSpec> = None;

    for conjunct in conjuncts {
        let binary = conjunct.as_any().downcast_ref::<BinaryExpr>()?;
        ...
        let left = as_filter_column_ref(binary.left(), filter)?;
        let right = as_filter_column_ref(binary.right(), filter)?;

        // cross comparator: streamed(left side of join) vs buffered(right side of join)
        if left.side == JoinSide::Left && right.side == JoinSide::Right {
            let streamed_field = streamed_schema.field(left.index);
            let buffered_field = buffered_schema.field(right.index);

            if !supported_fast_path_types(
                streamed_field.data_type(),
                buffered_field.data_type(),
            ) {
                return None;
            }

            if cross_spec.is_some() {
                return None;
            }

            cross_spec = Some(CrossComparatorSpec {
                kind,
                streamed_col_idx: left.index,
                buffered_col_idx: right.index,
            });
            continue;
        }

        if left.side == JoinSide::Right && right.side == JoinSide::Left {
            let streamed_field = streamed_schema.field(right.index);
            let buffered_field = buffered_schema.field(left.index);
            ...
            cross_spec = Some(CrossComparatorSpec {
                ...
                streamed_col_idx: right.index,
                buffered_col_idx: left.index,
            });
            continue;
        }

        // right-only comparator: buffered vs buffered
        if let Some(pref) = classify_right_only_cmp(conjunct, filter, buffered_schema) {
            if right_prefilter.is_some() {
                return None;
            }
            right_prefilter = Some(pref);
            continue;
        }

        // anything else (left-only, constants, complex exprs) is not safe for this fast path
        return None;
    }

    let cross = cross_spec?;
    Some(SemiAntiFastPathSpec {
        cross,
        right_prefilter,
    })
}
...
pub struct SemiAntiFastPathSpec {
    pub cross: CrossComparatorSpec,
    pub right_prefilter: Option<RightOnlyComparatorSpec>,
}

#[derive(Debug, Clone)]
pub struct CrossComparatorSpec {
    pub kind: SemiAntiFastPathKind,
    pub streamed_col_idx: usize,
    pub buffered_col_idx: usize,
}

#[derive(Debug, Clone)]
pub struct RightOnlyComparatorSpec {
    pub kind: SemiAntiFastPathKind,
    pub left_buffered_col_idx: usize,
    pub right_buffered_col_idx: usize,
}

#[derive(Debug, Clone)]
struct PranavBufferedBatchCache {
    range: Range<usize>,
    cross_values: ArrayRef,
    right_prefilter_left: Option<ArrayRef>,
    right_prefilter_right: Option<ArrayRef>,
}

#[derive(Debug, Clone, Copy)]
struct FilterColumnRef {
    side: JoinSide,
    index: usize,
}

fn as_filter_column_ref(
    expr: &PhysicalExprRef,
    filter: &JoinFilter,
) -> Option<FilterColumnRef> {
    let col = peel_casts(expr).as_any().downcast_ref::<Column>()?;
    let mapped = filter.column_indices().get(col.index())?;
    Some(FilterColumnRef {
        side: mapped.side,
        index: mapped.index,
    })
}
...
fn flatten_conjuncts<'a>(expr: &'a PhysicalExprRef, out: &mut Vec<&'a PhysicalExprRef>) {
...
fn eval_cross_cmp(ord: Ordering, kind: SemiAntiFastPathKind) -> bool {
    match kind {
        SemiAntiFastPathKind::Lt => ord == Ordering::Less,
        SemiAntiFastPathKind::LtEq => matches!(ord, Ordering::Less | Ordering::Equal),
        SemiAntiFastPathKind::Gt => ord == Ordering::Greater,
        SemiAntiFastPathKind::GtEq => matches!(ord, Ordering::Greater | Ordering::Equal),
        SemiAntiFastPathKind::NotEq => ord != Ordering::Equal,
        SemiAntiFastPathKind::None => false,
    }
}

fn classify_right_only_cmp(
    expr: &PhysicalExprRef,
    filter: &JoinFilter,
    buffered_schema: &SchemaRef,
) -> Option<RightOnlyComparatorSpec> {
    ...
    if left.side != JoinSide::Right || right.side != JoinSide::Right {
        return None;
    }

    let left_field = buffered_schema.field(left.index);
    let right_field = buffered_schema.field(right.index);

    if !supported_fast_path_types(left_field.data_type(), right_field.data_type()) {
        return None;
    }

    Some(RightOnlyComparatorSpec {
        kind,
        left_buffered_col_idx: left.index,
        right_buffered_col_idx: right.index,
    })
}
...
fn compare_array_values(
    ...
    macro_rules! compare_ord {
        ($T:ty) => {{
            let left = left_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast left array".to_string())
            })?;
            let right = right_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast right array".to_string())
            })?;
            Ok(left.value(left_idx).cmp(&right.value(right_idx)))
        }};
    }

    macro_rules! compare_ord_total_f32 {
        () => {{
            let left = left_array.as_any().downcast_ref::<Float32Array>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast left Float32Array".to_string())
            })?;
            let right = right_array.as_any().downcast_ref::<Float32Array>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast right Float32Array".to_string())
            })?;
            Ok(left.value(left_idx).total_cmp(&right.value(right_idx)))
        }};
    }

    macro_rules! compare_ord_total_f64 {
        () => {{
            let left = left_array.as_any().downcast_ref::<Float64Array>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast left Float64Array".to_string())
            })?;
            let right = right_array.as_any().downcast_ref::<Float64Array>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast right Float64Array".to_string())
            })?;
            Ok(left.value(left_idx).total_cmp(&right.value(right_idx)))
        }};
    }

    macro_rules! compare_ord_str {
        ($T:ty) => {{
            let left = left_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast left string array".to_string())
            })?;
            let right = right_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast right string array".to_string())
            })?;
            Ok(left.value(left_idx).cmp(right.value(right_idx)))
        }};
    }

    macro_rules! compare_ord_bin {
        ($T:ty) => {{
            let left = left_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast left binary array".to_string())
            })?;
            let right = right_array.as_any().downcast_ref::<$T>().ok_or_else(|| {
                DataFusionError::Internal("Failed to downcast right binary array".to_string())
            })?;
            Ok(left.value(left_idx).cmp(right.value(right_idx)))
        }};
    }
    ...
    if left_array.data_type() != right_array.data_type() {
        return not_impl_err!(
            "Pranav fast path requires identical logical types, got {} vs {}",
    ...
    match left_array.data_type() {
        DataType::Boolean => compare_ord!(BooleanArray),
        DataType::Int8 => compare_ord!(Int8Array),
        DataType::Int16 => compare_ord!(Int16Array),
        DataType::Int32 => compare_ord!(Int32Array),
        DataType::Int64 => compare_ord!(Int64Array),
        DataType::UInt8 => compare_ord!(UInt8Array),
        DataType::UInt16 => compare_ord!(UInt16Array),
        DataType::UInt32 => compare_ord!(UInt32Array),
        DataType::UInt64 => compare_ord!(UInt64Array),
        DataType::Float32 => compare_ord_total_f32!(),
        DataType::Float64 => compare_ord_total_f64!(),
        DataType::Utf8 => compare_ord_str!(StringArray),
        DataType::Utf8View => compare_ord_str!(StringViewArray),
        DataType::LargeUtf8 => compare_ord_str!(LargeStringArray),
        DataType::Binary => compare_ord_bin!(BinaryArray),
        DataType::BinaryView => compare_ord_bin!(BinaryViewArray),
        DataType::FixedSizeBinary(_) => compare_ord_bin!(FixedSizeBinaryArray),
        DataType::LargeBinary => compare_ord_bin!(LargeBinaryArray),
        DataType::Decimal32(..) => compare_ord!(Decimal32Array),
        DataType::Decimal64(..) => compare_ord!(Decimal64Array),
        DataType::Decimal128(..) => compare_ord!(Decimal128Array),
        DataType::Decimal256(..) => compare_ord!(Decimal256Array),
        DataType::Timestamp(time_unit, None) => match time_unit {
            TimeUnit::Second => compare_ord!(TimestampSecondArray),
            TimeUnit::Millisecond => compare_ord!(TimestampMillisecondArray),
            TimeUnit::Microsecond => compare_ord!(TimestampMicrosecondArray),
            TimeUnit::Nanosecond => compare_ord!(TimestampNanosecondArray),
        },
        DataType::Date32 => compare_ord!(Date32Array),
        DataType::Date64 => compare_ord!(Date64Array),
        dt => not_impl_err!("Unsupported Pranav fast-path type {}", dt),
    }
}

fn streamed_row_has_pranav_fast_match(
    spec: &SemiAntiFastPathSpec,
    buffered_batches_cache: &[PranavBufferedBatchCache],
    streamed_batch: &StreamedBatch,
    streamed_row_idx: usize,
) -> Result<bool> {
    let streamed_values = streamed_batch.batch.column(spec.cross.streamed_col_idx);

    if streamed_values.is_null(streamed_row_idx) {
        return Ok(false);
    }

    for buffered_batch in buffered_batches_cache.iter() {
        let cross_values = &buffered_batch.cross_values;
        let right_prefilter_left = buffered_batch.right_prefilter_left.as_ref();
        let right_prefilter_right = buffered_batch.right_prefilter_right.as_ref();

        for row_idx in buffered_batch.range.clone() {
            if let Some(pref) = &spec.right_prefilter {
                let lhs = right_prefilter_left.unwrap();
                let rhs = right_prefilter_right.unwrap();

                if lhs.is_null(row_idx) || rhs.is_null(row_idx) {
                    continue;
                }

                let ord = compare_array_values(lhs, row_idx, rhs, row_idx)?;
                if !eval_cross_cmp(ord, pref.kind) {
                    continue;
                }
            }

            if cross_values.is_null(row_idx) {
                continue;
            }

            let ord =
                compare_array_values(streamed_values, streamed_row_idx, cross_values, row_idx)?;
            if eval_cross_cmp(ord, spec.cross.kind) {
                return Ok(true);
            }
        }
    }
...
fn peel_casts<'a>(expr: &'a PhysicalExprRef) -> &'a PhysicalExprRef {
    if let Some(cast) = expr.as_any().downcast_ref::<CastExpr>() {
        return peel_casts(cast.expr());
    }
    if let Some(cast) = expr.as_any().downcast_ref::<TryCastExpr>() {
        return peel_casts(cast.expr());
    }
    expr
}
...
fn same_decimal_type(left: &DataType, right: &DataType) -> bool {
    match (left, right) {
        (DataType::Decimal128(lp, ls), DataType::Decimal128(rp, rs)) => lp == rp && ls == rs,
        (DataType::Decimal256(lp, ls), DataType::Decimal256(rp, rs)) => lp == rp && ls == rs,
        _ => false,
    }
}
...
/// Pranav's code
fn read_spilled_batch(spill_file: &RefCountedTempFile) -> Result<RecordBatch> {
    ...
    match batches.as_slice() {
        [] => internal_err!("Spilled SMJ batch file was empty"),
        [single] => Ok(single.clone()),
        many => Ok(concat_batches(&many[0].schema(), many)?),
    }
}

fn buffered_batch_record_batch(buffered_batch: &BufferedBatch) -> Result<RecordBatch> {
    match &buffered_batch.batch {
        BufferedBatchState::InMemory(batch) => Ok(batch.clone()),
        BufferedBatchState::Spilled(spill_file) => read_spilled_batch(spill_file),
    }
}
...
pub(super) struct SortMergeJoinStream {
    ...
    pub pranav_buffered_batches_cache: Option<Vec<PranavBufferedBatchCache>>,
    pub pranav_output_indices: UInt64Builder,
}
...
impl SortMergeJoinStream {
    ...
    pub fn try_new(
        ...
        Ok(Self {
            ...
            reservation,
            runtime_env,
            pranav_fast_path_spec,
            pranav_buffered_batches_cache: None,
            pranav_output_indices: UInt64Builder::new(),
            spill_manager,
    ...
    fn needs_deferred_filtering(&self) -> bool {
        if self.pranav_fast_path_spec.is_some()
    ...
    fn join_partial_pranav_semi_anti(&mut self) -> Result<bool> {
        if !matches!(self.current_ordering, Ordering::Equal) {
            return Ok(false);
        }
        ...
        if self.pranav_fast_path_spec.is_none() {
            
            return Ok(false);
        }
        
        self.ensure_pranav_buffered_batches_cache()?;
        
        let spec = self.pranav_fast_path_spec.as_ref().ok_or_else(|| {
            DataFusionError::Internal("Pranav fast path spec missing".to_string())
        })?;
        
        let buffered_batches_cache = self.pranav_buffered_batches_cache.as_ref().ok_or_else(|| {
            DataFusionError::Internal("Pranav buffered batches cache missing".to_string())
        })?;
        
        let exists = streamed_row_has_pranav_fast_match(
            spec,
            buffered_batches_cache,
            &self.streamed_batch,
        ...
        if should_emit {
            self.append_current_streamed_row_to_pranav_output()?;
        }
    ...
    fn poll_buffered_batches(&mut self, cx: &mut Context) -> Poll<Option<Result<()>>> {
        loop {
            match &self.buffered_state {
                BufferedState::Init => {
                    self.reset_pranav_buffered_batches_cache();
    ...
    fn reset_pranav_buffered_batches_cache(&mut self) {
        self.pranav_buffered_batches_cache = None;
    }
    
    fn ensure_pranav_buffered_batches_cache(&mut self) -> Result<()> {
        if self.pranav_fast_path_spec.is_none() || self.pranav_buffered_batches_cache.is_some() {
            return Ok(());
        }
    
        let spec = self.pranav_fast_path_spec.as_ref().unwrap();
        let mut caches = Vec::with_capacity(self.buffered_data.batches.len());
    
        for buffered_batch in self.buffered_data.batches.iter() {
            let batch = buffered_batch_record_batch(buffered_batch)?;
    
            caches.push(PranavBufferedBatchCache {
                range: buffered_batch.range.clone(),
                cross_values: Arc::clone(batch.column(spec.cross.buffered_col_idx)),
                right_prefilter_left: spec
                    .right_prefilter
                    .as_ref()
                    .map(|p| Arc::clone(batch.column(p.left_buffered_col_idx))),
                right_prefilter_right: spec
                    .right_prefilter
                    .as_ref()
                    .map(|p| Arc::clone(batch.column(p.right_buffered_col_idx))),
            });
        }
    
        self.pranav_buffered_batches_cache = Some(caches);
        Ok(())
    }

    fn append_current_streamed_row_to_pranav_output(&mut self) -> Result<()> {
        self.pranav_output_indices.append_value(self.streamed_batch.idx as u64);
    
        if self.pranav_output_indices.len() >= self.batch_size {
            self.flush_pranav_output()?;
        }
        Ok(())
    }
    
    fn flush_pranav_output(&mut self) -> Result<()> {
        let left_indices = self.pranav_output_indices.finish();
        ...
        let batch = RecordBatch::try_new(Arc::clone(&self.streamed_schema), columns)?;
        self.output.push_batch(batch).expect("Failed to push Pranav output batch");
        Ok(())
    }
    ...
    fn join_partial(&mut self) -> Result<()> {
        ...
        if self.join_partial_pranav_semi_anti()? {
            return Ok(());
        }
