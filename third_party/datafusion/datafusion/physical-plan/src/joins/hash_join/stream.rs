...
use arrow::array::{Array, ArrayRef, BooleanArray, BooleanBufferBuilder, UInt32Array, UInt64Array};
...
fn filter_null_aware_anti_probe_batch(
    batch: RecordBatch,
    values: &[ArrayRef],
) -> Result<RecordBatch> {
    let mut mask = BooleanBufferBuilder::new(batch.num_rows());
    for row_idx in 0..batch.num_rows() {
        let keep = values.iter().all(|arr| !arr.is_null(row_idx));
        mask.append(keep);
    }
    let mask = BooleanArray::new(mask.finish(), None);
...
impl HashJoinStream {
    ...
    fn fetch_probe_batch(
        ...
        match ready!(self.right.poll_next_unpin(cx)) {
            ...
            Some(Ok(batch)) => {
                ...
                if self.is_null_aware_anti_join && matches!(self.join_type, JoinType::LeftAnti | JoinType::RightAnti) {
                    let build_side = self.build_side.try_as_ready()?;
                    if !build_side.left_data.hash_map().is_empty()
                        && !build_side.left_data.build_has_null_key
                    {
                        let raw_values = evaluate_expressions_to_arrays(&self.on_right, &batch)?;
                        batch = filter_null_aware_anti_probe_batch(batch, &raw_values)?;
                        if batch.num_rows() == 0 {
                            self.state = HashJoinStreamState::FetchProbeBatch;
    ...
    fn process_probe_batch(
        ...
        self.join_metrics
            ...
            if self.is_null_aware_anti_join && matches!(self.join_type, JoinType::LeftAnti | JoinType::RightAnti) {
                if build_side.left_data.build_has_null_key {
                    timer.done();
                    self.state = HashJoinStreamState::FetchProbeBatch;
