...
import org.apache.comet.serde.QueryPlanSerde.{aggExprToProto, exprToProto, serializeDataType, supportedSortType}
import org.apache.comet.serde.operator.CometSink

object CometCartesianProductExec
    extends CometOperatorSerde[org.apache.spark.sql.execution.joins.CartesianProductExec] {

  override def enabledConfig: Option[ConfigEntry[Boolean]] = None

  override def convert(
      op: org.apache.spark.sql.execution.joins.CartesianProductExec,
      builder: OperatorOuterClass.Operator.Builder,
      childOp: OperatorOuterClass.Operator*): Option[OperatorOuterClass.Operator] = {
    if (childOp.length != 2) {
      withInfo(op, s"CartesianProduct requires 2 children, found ${childOp.length}")
      return None
    }

    val cartesianBuilder = OperatorOuterClass.CartesianProduct.newBuilder()

    op.condition.foreach { cond =>
      val condProto = exprToProto(cond, op.left.output ++ op.right.output)
      if (condProto.isEmpty) {
        withInfo(op, cond)
        return None
      }
      cartesianBuilder.setCondition(condProto.get)
    }

    Some(builder.setCartesianProduct(cartesianBuilder).build())
  }

  override def createExec(
      nativeOp: OperatorOuterClass.Operator,
      op: org.apache.spark.sql.execution.joins.CartesianProductExec): CometNativeExec = {
    CometCartesianProductExec(nativeOp, op, op.left, op.right, SerializedPlan(None))
  }
}
    ...
    if (leftKeys.forall(_.isDefined) &&
      ...
      join match {
        case bhj: BroadcastHashJoinExec =>
          joinBuilder.setIsNullAwareAntiJoin(bhj.isNullAwareAntiJoin)
        case _ =>
      }
...
object CometBroadcastNestedLoopJoinExec
    extends CometOperatorSerde[org.apache.spark.sql.execution.joins.BroadcastNestedLoopJoinExec] {

  override def enabledConfig: Option[ConfigEntry[Boolean]] = None

  override def convert(
      op: org.apache.spark.sql.execution.joins.BroadcastNestedLoopJoinExec,
      builder: OperatorOuterClass.Operator.Builder,
      childOp: OperatorOuterClass.Operator*): Option[OperatorOuterClass.Operator] = {
    if (childOp.length != 2) {
      withInfo(op, s"BroadcastNestedLoopJoin requires 2 children, found ${childOp.length}")
      return None
    }

    op.joinType match {
      case Cross =>
      case Inner if op.condition.isEmpty =>
      case _ =>
        withInfo(op, s"BroadcastNestedLoopJoin with join type ${op.joinType} is not supported")
        return None
    }
  ...
  override def createExec(
      nativeOp: OperatorOuterClass.Operator,
      op: org.apache.spark.sql.execution.joins.BroadcastNestedLoopJoinExec): CometNativeExec = {
...
case class CometBroadcastNestedLoopJoinExec(
  ...
  override def stringArgs: Iterator[Any] =
    Iterator(buildSide, joinType, condition, left, right)
}
